...
jest.mock('@apollo/client/link/subscriptions', () => {
  const originalModule = jest.requireActual('@apollo/client/link/subscriptions');
...
test("sends query over HTTP", () => {
  const lastLink = new ApolloLink(jest.fn(operation => {
  ...
  executeRequest(gql`query { foo }`, link);
...
test("sends mutations over HTTP", () => {
  ...
  executeRequest(gql`mutation { foo }`, link);
  expect(lastLink.request).toHaveBeenCalled()
});
...
test("sends subscriptions over ws", () => {
  ...
  executeRequest(gql`subscription { foo }`, link);
  expect(lastLink.request).toHaveBeenCalled()
});
