...
function HomePageComponent({ tableColumns }) {
  ...
  const scoutPlannedTable =
    scoutPlannedTours?.getTours?.map((tour) => [
      tour.requested_by.name,
      tour.tour_address,
      tour.date_requested
        ? new Date(tour.date_requested).toLocaleDateString()
        : "N/A",
      tour.date_completed
        ? new Date(tour.date_completed).toLocaleDateString()
        : "N/A",
    ]) ?? [];
  ...
  const scoutCompletedTable =
    scoutCompletedTours?.getTours?.map((tour) => [
      tour.requested_by.name,
      tour.tour_address,
      tour.date_requested
        ? new Date(tour.date_requested).toLocaleDateString()
  ...
  const requesterPlannedTable =
    requesterPlannedTours?.getTours?.map((tour) => [
  ...
  const requesterCompletedTable =
    requesterCompletedTours?.getTours?.map((tour) => [
      tour.scouted_by.name,
      tour.tour_address,
      tour.date_requested
        ? new Date(tour.date_requested).toLocaleDateString()
    ...
    <div style={{ flexGrow: 1 }}>
      <div className="userTypeContainer">
        ...
        */}
        ...
        <button
          className={`userTypeButton1 ${
            activeButton === "requester" ? "active" : "inactive"
          }`}
          onClick={() => handleButtonClick("requester")}
        >
          Tours as a requester
        </button>
        <button
          className={`userTypeButton2 ${
            activeButton === "scout" ? "active" : "inactive"
          }`}
          onClick={() => handleButtonClick("scout")}
        >
          Tours as a scout
        </button>
      </div>
      ...
      {activeButton === "scout" &&
        scoutPlannedTours?.getTours?.map((tour, idxTour) => (
          <Box
            key={tour.tour_id}
            sx={{
              margin: "0 auto",
            ...
            <Grid container spacing={2}>
              {tableColumns.map((_, idxCol) => (
                <Grid item xs={2} key={idxCol}>
                  ...
                  <Item sx={{ marginLeft: "10px" }}>
                    {scoutPlannedTable[idxTour][idxCol]}
                  </Item>
                </Grid>
              ))}
              <Grid
              ...
              >
                <Button
                  variant="contained"
                  onClick={() => navigateToDetails(tour.tour_id)}
                >
                  View Tour
                </Button>
              </Grid>
      ...
      {activeButton === "requester" &&
        requesterPlannedTours?.getTours?.map((tour, idxTour) => (
          <Box
            key={tour.tour_id}
            sx={{
              margin: "0 auto",
            ...
            <Grid container spacing={2}>
              {tableColumns.map((_, idxCol) => (
                <Grid item xs={2} key={idxCol}>
                  ...
                  <Item sx={{ marginLeft: "10px" }}>
                    {requesterPlannedTable[idxTour][idxCol]}
                  </Item>
                </Grid>
              ))}
              <Grid
      ...
      {activeButton === "scout" &&
        scoutCompletedTours?.getTours?.map((tour, idxTour) => (
          <Box
            key={tour.tour_id}
            sx={{
              margin: "0 auto",
            ...
            <Grid container spacing={2}>
              {tableColumns.map((_, idxCol) => (
                <Grid item xs={2} key={idxCol}>
                  ...
                  <Item sx={{ marginLeft: "10px" }}>
                    {scoutCompletedTable[idxTour][idxCol]}
                  </Item>
                </Grid>
              ))}
              <Grid
      ...
      {activeButton === "requester" &&
        requesterCompletedTours?.getTours?.map((tour, idxTour) => (
          <Box
            key={tour.tour_id}
            sx={{
              margin: "0 auto",
            ...
            <Grid container spacing={2}>
              {tableColumns.map((_, idxCol) => (
                <Grid item xs={2} key={idxCol}>
                  ...
                  <Item sx={{ marginLeft: "10px" }}>
                    {requesterCompletedTable[idxTour][idxCol]}
                  </Item>
                </Grid>
              ))}
              <Grid
