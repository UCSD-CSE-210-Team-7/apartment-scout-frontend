...
const Sidebar = ({
  ...
  sortable.sort(
    (a, b) =>
      new Date(b.messages[b.messages.length - 1]?.msg_time) -
      new Date(a.messages[a.messages.length - 1]?.msg_time)
  );
        ...
        <div
          ...
          style={
            conversation.conversation_id === selectedConversation
              ? { background: "#ADE8F4", cursor: "pointer" }
              : { cursor: "pointer" }
          }
          className="userChat"
        >
          ...
          <div className="userChatInfo">
            ...
            {conversation.last_msg && (
              <p>
                {(conversation.last_msg.sender.email === user ? "You: " : "") +
