...
test("render with conversation", async () => {
  const { asFragment } = render( 
    <Chat conversation={data.me.conversations[0]} sendMessage={() => null} loading={false} />, 
  )
  expect(asFragment()).toMatchSnapshot()
});
...
test("send new message", async () => {
  ...
  const { asFragment } = render( 
    <Chat conversation={data.me.conversations[0]} sendMessage={sendMessage} loading={false} />, 
  );
